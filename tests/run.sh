...
$MYGREP 'foo[0-9]' data/eg_probe.txt data/no_newline.txt data
...
$MYGREP -E -i -e '^((ab|cd)+e?f*){2,4}$' -e 'a{2,}' -e 'b{0}' -e 'c{0,2}' -f data/ere_edges.txt -f data/ere_edges_extra.txt -f data/ere_spikes.txt data/engine.txt data/classmix.txt data/lex_input.txt
$MYGREP -G '^\<\(\w\{2,4\}\)\>\W\1$\|^\(\w\)\B\w\b\W\2$' data/runtime_regex.txt
$MYGREP -F -f data/long_pattern.txt data/large.txt
$MYGREP -G -f data/boundary_extend.txt -f data/macro_extend.txt -f data/compile_spikes.txt data/empty.txt
...
printf 'aaaaaaaaaaaaaaaaaaaaaaaa\nB\nC\nxx\nD\naaE\naaaaE\n' | $MYGREP -G -c -l -f data/precision_pattern.txt
...
$MYGREP -G -f data/macro_ladder.txt -f data/compile_spikes.txt data/empty.txt